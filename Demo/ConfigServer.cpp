...
void CConfigServer::GetConfig()
{
	...
	DWORD retlen = 0;
	BOOL bRet = CLIENT_GetDevConfig(m_pDev->LoginID, RV_DEV_DEVICECFG, 0,
...
/* DSP能力描述 
* DSP abbility discription
typedef struct 
{
	DWORD	dwVideoStandardMask;	//视频制式掩码，按位表示设备能够支持的视频制式
									//mask of video system, mark supported video systems by bits 
	DWORD	dwImageSizeMask;		//分辨率掩码，按位表示设备能够支持的分辨率设置
									//mask of resolve, mark supported resolves by bits
	DWORD	dwEncodeModeMask;		//编码模式掩码，按位表示设备能够支持的编码模式设置
									//mask of code mode, mark supported code modes by bits
...
void CConfigServer::OnApply() 
{
	...
	BOOL bRet = CLIENT_SetDevConfig(m_pDev->LoginID, RV_DEV_DEVICECFG, 0,
