...
void CUser_ModifyUser::OnBtnOk() 
{
	...
	GetDlgItem(IDC_NAME_NEW_EDIT)->GetWindowText(newInfo.name, RV_USER_NAME_LENGTH);
	memcpy(newInfo.passWord, m_user_info->userList[m_userIdx].passWord, RV_USER_PSW_LENGTH);
	GetDlgItem(IDC_MEMO_NEW_EDIT)->GetWindowText(newInfo.memo, RV_MEMO_LENGTH);
	...
	BOOL bRet = CLIENT_OperateUserInfoEx(m_dev->LoginID, 5, (void *)&newInfo, (void *)&m_user_info->userList[m_userIdx], 1000);
	if (!bRet)
	{
