...
BOOL CAutoMaintenance::OnInitDialog() 
{
	...
	{
		DWORD ret = 0;
		BOOL b = CLIENT_GetDevConfig(m_dev->LoginID, RV_DEV_AUTOMTCFG, 0, &m_atC, sizeof(DHDEV_AUTOMT_CFG), &ret, CONFIG_WAITTIME);
...
void CAutoMaintenance::OnOK() 
{
	if (m_dev != NULL)
	{
		...
		BOOL b = CLIENT_SetDevConfig(m_dev->LoginID, RV_DEV_AUTOMTCFG, 0, &m_atC, sizeof(m_atC), CONFIG_WAITTIME);
		if (!b)
		{
