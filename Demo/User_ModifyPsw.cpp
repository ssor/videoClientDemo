...
void CUser_ModifyPsw::OnBtnOk() 
{
	...
	char tmp1[RV_USER_PSW_LENGTH] = {0};
	char tmp2[RV_USER_PSW_LENGTH] = {0};
	GetDlgItem(IDC_NEW_PSW_EDIT1)->GetWindowText(tmp1, RV_USER_PSW_LENGTH);
	GetDlgItem(IDC_NEW_PSW_EDIT2)->GetWindowText(tmp2, RV_USER_PSW_LENGTH);
	...
	memcpy(newInfo.passWord, tmp1, RV_USER_PSW_LENGTH);

	memcpy(oldInfo.name, m_user_info->userList[m_userIdx].name, RV_USER_PSW_LENGTH);
	GetDlgItem(IDC_OLD_PSW_EDIT)->GetWindowText(tmp1, RV_USER_PSW_LENGTH);
	memcpy(oldInfo.passWord, tmp1, RV_USER_PSW_LENGTH);

	BOOL bRet = CLIENT_OperateUserInfoEx(m_dev->LoginID, 6, (void *)&newInfo, &oldInfo, 1000);
	if (!bRet)
	{
