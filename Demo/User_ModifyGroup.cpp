...
{
	USER_GROUP_INFO_EX gpInfo = {0};
	gpInfo.dwID = m_user_info->groupList[m_gpIdx].dwID;
	GetDlgItem(IDC_GPNAME_NEW_EDIT)->GetWindowText(gpInfo.name, RV_USER_NAME_LENGTH);
	GetDlgItem(IDC_GPMEMO_NEW_EDIT)->GetWindowText(gpInfo.memo, RV_MEMO_LENGTH);
	...
	BOOL bRet = CLIENT_OperateUserInfoEx(m_dev->LoginID, 2, &gpInfo, &m_user_info->groupList[m_gpIdx], 1000);
	if (!bRet)
	{
