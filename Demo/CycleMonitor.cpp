...
int CCycleMonitor::InitLeftTreeCallBack_Imp(const DeviceNode& node)
{
	...
	for(int chlCounter = 0; chlCounter < node.Info.byChanNum; chlCounter++)
	{
		...
		//only show the channels that not in tour list and not being watching
		BOOL found = FALSE;
		//在每个轮循画面窗口的参数中查询
		//query in the params of every tour window
...
BOOL CCycleMonitor::OnInitDialog() 
{
	...
	//确定属性页子窗口的位置
	//affirm the position of property page window
	CRect rect;
	...
	{
		//初始化属性页各子窗口
		//initialize the sub windows of property page
		CString childtitle;
	...
	//初始化设备和通道列表
	//initialize the device and channel list
...
void CCycleMonitor::OnAdd1() 
{
	...
	//the picked-out "item data" maybe not the Device node, so that's no problem if not read it's member while judge
	...
	{
		//将整个设备的表项都添加到右边轮循列表
		//add the whole device to the right tour list
		int addindex = 0;
...
void CCycleMonitor::DeleteItem(int rightindex)
{
	...
	{	
		//查找正确的插入位置
		//seach the correct insert position
		...
		int leftindex = m_curRow;//initialize to be the final position//初始化为最后一个位置
