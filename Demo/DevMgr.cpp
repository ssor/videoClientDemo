		...
		case RV_COMM_ALARM:
			{
				...
				//设备列表中信息刷新
				//refresh the infomation of device list
				...
				thisNode->alarmType = RV_COMM_ALARM;
		...
		case RV_SHELTER_ALARM:
			{
				...
				thisNode->alarmType = RV_SHELTER_ALARM;
				memcpy(&thisNode->state, &(*it)->State, sizeof(DEV_STATE));
		...
		case RV_DISK_FULL_ALARM:
			{
				...
				thisNode->alarmType = RV_DISK_FULL_ALARM;
				memcpy(&thisNode->state, &(*it)->State, sizeof(DEV_STATE));
		...
		case RV_DISK_ERROR_ALARM:
			{
				...
				thisNode->alarmType = RV_DISK_ERROR_ALARM;
				memcpy(&thisNode->state, &(*it)->State, sizeof(DEV_STATE));
		...
		case RV_SOUND_DETECT_ALARM:
			{
				...
				thisNode->alarmType = RV_SOUND_DETECT_ALARM;
				memcpy(&thisNode->state, &(*it)->State, sizeof(DEV_STATE));

				nRet = 0;
				break;
			}
		case RV_ALARM_DECODER_ALARM:
			{
				if (dwBufLen != 16)
				{
					nRet = -1;
				...
				thisNode->alarmType = RV_ALARM_DECODER_ALARM;
				memcpy(&thisNode->state, &(*it)->State, sizeof(DEV_STATE));
				nRet = 0;
			}
			break;
		case RV_REBOOT_EVENT_EX:
			{
