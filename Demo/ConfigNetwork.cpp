...
void CConfigNetwork::GetConfig()
{
	...
	BOOL bRet = FALSE;
	DWORD retlen = 0;
	bRet = CLIENT_GetDevConfig(m_pDev->LoginID, RV_DEV_DEVICECFG, 0,
	...
	bRet = CLIENT_GetDevConfig(m_pDev->LoginID, RV_DEV_NETCFG, 0,
	...
	//DDNS config
	bRet = FALSE;
	bRet = CLIENT_GetDevConfig(m_pDev->LoginID, RV_DEV_MULTI_DDNS, 0,
...
RV_REMOTE_HOST *CConfigNetwork::GetRemoteHostType(int hostIdx, BOOL bChg)
{
	RV_REMOTE_HOST *ret = 0;
	switch(hostIdx)
	{
	case 0:	//server of alarm /* 报警服务器 */
		ret = &m_netCFG.struAlarmHost;
		break;
	case 1://server of log /* 日志服务器 */
		ret = &m_netCFG.struLogHost;
		break;
	case 2://server of smtp /* SMTP服务器 */
		ret = &m_netCFG.struSmtpHost;
		break;
	case 3://group of multicast /* 多播组 */
		ret = &m_netCFG.struMultiCast;
		break;
	case 4://server of NFS /* NFS服务器 */
		ret = &m_netCFG.struNfs;
		break;
	case 5://server of PPPOE /* PPPoE服务器 */
...
void CConfigNetwork::OnApply() 
{
	...
	GetDlgItemText(IDC_EDIT_DEVNAME, m_netCFG.sDevName, RV_MAX_NAME_LEN);
	...
	//ethernet config
	GetDlgItemText(IDC_EDIT_NETIP, m_netCFG.stEtherNet[m_etherIdx].sDevIPAddr, RV_MAX_IPADDR_LEN);
	GetDlgItemText(IDC_EDIT_NETIPMASK, m_netCFG.stEtherNet[m_etherIdx].sDevIPMask, RV_MAX_IPADDR_LEN);
	GetDlgItemText(IDC_EDIT_NETGATEWAY, m_netCFG.stEtherNet[m_etherIdx].sGatewayIP, RV_MAX_IPADDR_LEN);
	m_netCFG.stEtherNet[m_etherIdx].dwNetInterface = m_interfacesel.GetCurSel()+1;
	GetDlgItemText(IDC_EDIT_NETMAC, m_netCFG.stEtherNet[m_etherIdx].byMACAddr, RV_MACADDR_LEN);
	
	//remote config
	RV_REMOTE_HOST *thishost = 0;
	...
	GetDlgItemText(IDC_EDIT_HOSTIP, thishost->sHostIPAddr, RV_MAX_IPADDR_LEN);
	thishost->wHostPort = GetDlgItemInt(IDC_EDIT_HOSTPORT);
	GetDlgItemText(IDC_EDIT_HOSTUSERNAME, thishost->sHostUser, RV_MAX_HOST_NAMELEN);
	GetDlgItemText(IDC_EDIT_HOSTUSERPSW, thishost->sHostPassword, RV_MAX_HOST_PSWLEN);
	if (7 == m_hostIdx)
	{
		GetDlgItemText(IDC_EDIT_HOSTRESERVED, m_netCFG.sDdnsHostName, RV_MAX_HOST_NAMELEN);
	}
	
	//mail config
	GetDlgItemText(IDC_EDIT_MAILUSERNAME, m_netCFG.struMail.sUserName, RV_MAX_NAME_LEN);
	GetDlgItemText(IDC_EDIT_MAILUSERPSW, m_netCFG.struMail.sUserPsw, RV_MAX_NAME_LEN);
	GetDlgItemText(IDC_EDIT_MAILIP, m_netCFG.struMail.sMailIPAddr, RV_MAX_IPADDR_LEN);
	m_netCFG.struMail.wMailPort = GetDlgItemInt(IDC_EDIT_MAILPORT);
	GetDlgItemText(IDC_EDIT_MAILDESADDR, m_netCFG.struMail.sDestAddr, RV_MAX_MAIL_ADDR_LEN);
	GetDlgItemText(IDC_EDIT_MAILCCADDR, m_netCFG.struMail.sCcAddr, RV_MAX_MAIL_ADDR_LEN);
	GetDlgItemText(IDC_EDIT_MAILBCCADDR, m_netCFG.struMail.sBccAddr, RV_MAX_MAIL_ADDR_LEN);
	GetDlgItemText(IDC_EDIT_MAILSUBJECT, m_netCFG.struMail.sSubject, RV_MAX_MAIL_ADDR_LEN);
	
//	strcpy(m_netCFG.struMail.sSenderAddr, "111111111111");
//
	//save
	BOOL bRet = CLIENT_SetDevConfig(m_pDev->LoginID, RV_DEV_NETCFG, 0,
	...
	else
	{
		...
		OnSelchangeComboDdnsid();
		bRet=CLIENT_SetDevConfig(m_pDev->LoginID,RV_DEV_MULTI_DDNS,0,&m_ddnsCFG,sizeof(DHDEV_MULTI_DDNS_CFG),CONFIG_WAITTIME);
		if(bRet==TRUE)
		{
...
void CConfigNetwork::OnSelchangeComboRemotehost() 
{
	...
	if (m_hostIdx >= 0)
	{
		...
		GetDlgItemText(IDC_EDIT_HOSTIP, prehost->sHostIPAddr, RV_MAX_IPADDR_LEN);
		prehost->wHostPort = GetDlgItemInt(IDC_EDIT_HOSTPORT);
		GetDlgItemText(IDC_EDIT_HOSTUSERNAME, prehost->sHostUser, RV_MAX_HOST_NAMELEN);
		GetDlgItemText(IDC_EDIT_HOSTUSERPSW, prehost->sHostPassword, RV_MAX_HOST_PSWLEN);
		if (7 == m_hostIdx)//DDNS config//DDNS配置
		{
			GetDlgItemText(IDC_EDIT_HOSTRESERVED, m_netCFG.sDdnsHostName, RV_MAX_HOST_NAMELEN);
		}
	}
...
void CConfigNetwork::OnSelchangeComboEthernet() 
{
	...
	if (etherIdx < 0 || etherIdx >= RV_MAX_ETHERNET_NUM) 
	{
		return;
	}
	m_etherIdx = etherIdx;
...
void CConfigNetwork::OnSelchangeComboDdnsid() 
{
	...
	{	
		...
		GetDlgItemText(IDC_EDIT_ALIAS,dns->szAlias,RV_MAX_DDNS_ALIAS_LEN);
		GetDlgItemText(IDC_EDIT_DOMAINNAME,dns->szDomainName,RV_MAX_DOMAIN_NAME_LEN);
		GetDlgItemText(IDC_EDIT_SERVERIP,dns->szServerIp,RV_MAX_IPADDR_LEN);
		GetDlgItemText(IDC_EDIT_SERVRRTYPE,dns->szServerType,RV_MAX_SERVER_TYPE_LEN);
