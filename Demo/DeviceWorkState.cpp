	...
	: CDialog(CDeviceWorkState::IDD, pParent)
{
	...
	memset(&m_diskState, 0 , sizeof(RV_HARDDISK_STATE));
}
...
*/	
	...
	BOOL bRet = CLIENT_QueryDevState(m_dev->LoginID, RV_DEVSTATE_COMM_ALARM,
	...
	bRet = CLIENT_QueryDevState(m_dev->LoginID, RV_DEVSTATE_SHELTER_ALARM,
	...
	//record state
	bRet = CLIENT_QueryDevState(m_dev->LoginID, RV_DEVSTATE_RECORDING,
	...
	bRet = CLIENT_QueryDevState(m_dev->LoginID, RV_DEVSTATE_RESOURCE, (char *)dwResource, 3*sizeof(DWORD), &retlen);
	...
	bRet = CLIENT_QueryDevState(m_dev->LoginID, RV_DEVSTATE_BITRATE, (char *)dwBitRate, channum*sizeof(DWORD), &retlen);
	...
	bRet = CLIENT_QueryDevState(m_dev->LoginID, RV_DEVSTATE_CONN, (char *)dwNetConn, 11*sizeof(DWORD), &retlen);
	...
	bRet = CLIENT_QueryDevState(m_dev->LoginID, RV_DEVSTATE_DISK,
								(char*)&m_diskState, sizeof(RV_HARDDISK_STATE), &retlen, 1000);
	if(!bRet || retlen != sizeof(RV_HARDDISK_STATE))
	{
		((CNetSDKDemoDlg *)GetParent())->LastError();//Zhengdh 06.11.24
		return FALSE;
	}
