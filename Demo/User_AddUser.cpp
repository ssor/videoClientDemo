...
{
	//construct user info
	USER_INFO_EX urInfo = {0};
	urInfo.dwID = m_user_info->dwUserNum + 1;
	GetDlgItem(IDC_NAME_EDIT)->GetWindowText(urInfo.name, RV_USER_NAME_LENGTH);
	GetDlgItem(IDC_PSW_EDIT)->GetWindowText(urInfo.passWord, RV_USER_PSW_LENGTH);
	GetDlgItem(IDC_MEMO_EDIT)->GetWindowText(urInfo.memo, RV_MEMO_LENGTH);
	...
	BOOL bRet = CLIENT_OperateUserInfoEx(m_dev->LoginID, 3/*type: add user*/, &urInfo, 0, 1000);
	if (!bRet)
	{
