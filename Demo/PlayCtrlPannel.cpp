...
void CPlayCtrlPannel::OnCtrlPlaystop() 
{
	...
	{
		//主窗口已有提示，这里暂时不需要处理。
		//there is prompt in main window, so no need to deal here
	}
}
...
void CPlayCtrlPannel::OnTimer(UINT nIDEvent) 
{
	int fileframe = 0;
	int playframe = 0;
	int pos = 0;
...
//	EnterCriticalSection(&g_csPlayPos);
	...
	//verify that the playing id is coresponding to the infomation recorded by current view
