...
/////////////////////////////////初始化回调相关的函数////////////////////////////////////////////////
/////////////////////////////////initialize relate functions of callback////////////////////////////////////////////////
//设备断开时回调函数，可以用来处理断开后设备列表的更新及设备的删除操作
//the callback function of device disconnection, used to deal the updation and deleting operations of device list after device disconnection and
...
//the function of message callback,  used to whole sdk apply
...
//deal with the device message, just defined alarm message callback now
...
//自定义画板回调,可以用来显示通道信息
//self-defined palette callback, this can be used to show channel infomation
...
// 	}
	...
	//初始化网络sdk,所有调用的开始
	//initialize network sdk, all call started with this
	...
	//设置信息回调函数,默认接收所有设备信息
	//set infomation callback function, and defaultly recv all device infomation
	...
	//初始化画面分割模式选择项
	//initialize image split mode options
...
//	m_bFullSCRN = FALSE;
	...
	//初始化关闭声音
	//initialize to close audio
	m_curSoundSplit = -1;
	
	//创建状态页面和系统配置页面
	//create status page and system config pages
	...
	//初始化云台控制状态
	//initialize PTZ control status
	m_bPTZCtrl = FALSE;

	//刷新界面信息
	//refresh UI infomation
...
////////////////////////////////内部使用小接口函数///////////////////////////////////////////////////
////////////////////////////////using littile interface functions///////////////////////////////////////////////////
//根据登录ID获取设备信息指针
//get device infomation pointer by logon id
...
//获取当前设备列表中选择的设备信息指针
//get device infomation poiter selected from device list
void *CNetSDKDemoDlg::GetCurDeviceInfo()
{
	...
	if(!node)
	{
		MessageBox(ConvertString(MSG_DEMODLG_CHECKSEL),"VisionDigi SDK Client");
		return NULL;
	}
	
	nData = m_devicelist.GetItemData(node);
	if(nData < 16)    //channel options, get father option//通道选项，取得父项
	{
		node = m_devicelist.GetParentItem(node);
	}

	return (void *)m_devicelist.GetItemData(node);  //the data recored by father item is the device infomation poiter value//父项记录的数据为设备的信息指针值
}

//根据句柄获取播放窗口序号,其中句柄可以是监视通道Id,播放Id,预览id等
//get window serial number by handle, and the handle can be watching channel Id, playing Id, previewing Id, etc...
int CNetSDKDemoDlg::GetHandleSplit(LONG lPlayHandle)
{
	SplitInfoNode siNode;
	memset(&siNode, 0, sizeof(siNode));
	for(int i = 0; i < RV_MAX_CHANNUM; i++)
	{
...
//get the view serial number appointed in current view split mode 
...
//检测当前画面状态并关闭当前状态
//detect current view status and close current status
...
//检测当前选择通道是否在某个轮循列表中
//detect current selected channel if is in some tour list
...
//关闭画面的显示状态
//close the display status of view
...
//	CCSLock lck(g_cs);
	...
	case SPLIT_TYPE_CYCLEMONITOR:
		...
		//only used to "close" command, "delete" and "disconnect" device have other code 
		KillTimer(scrNo);
...
int CNetSDKDemoDlg::UpdateDeviceListCallback_Imp(const DeviceNode& node)
{
	...
	//设备项直接将设备信息指针作为列表Id
	//device item directly treat device infomation pointer as Id of list
	...
	for(int j = 0; j < node.Info.byChanNum; j++)
	{
		...
		//通道项将通道序号作为列表ID
		//channel item treat channel serial number as ID of list
...
////////////////////////////////刷新显示相关的接口函数//////////////////////////////////////////
////////////////////////////////refresh display relate interface functions//////////////////////////////////////////
//刷新设备列表显示，直接根据g_ptrdevicelist重新刷新显示
//refresh device list display, directly refresh display by g_ptrdevicelist
...
//刷新当前显示画面的相关信息
//refresh relate infomation of current view
void CNetSDKDemoDlg::UpdateCurScreenInfo()
{
	//判断是否打开了语音对讲
	//judge if is opened talk
	SplitInfoNode siNode;
	memset(&siNode, 0, sizeof(siNode));
...
/*	BOOL bTalkOpen = FALSE;
	...
	*/
	...
	//更新保存数据项和音频项
	//refresh saved data item and audio item
...
//refresh video params control area, nMode 0~3 coresponding to single item refreshing
...
/////////////for one////////////////
//播放进度条拖动的处理
//the deal of playing process draging
...
void CNetSDKDemoDlg::AddDevice() 
{
	...
	if(dlg.DoModal() == IDOK)
	{
		UpdateDeviceList();	   //refresh device list display//刷新设备列表显示	
	}	
...
//real-time data callback, used to calculate bit-stream statistic and save data
...
//the callback function of replay data, and save data to file in demo
...
//get device Id by device list, and select channel index by channel
void CNetSDKDemoDlg::OpenChannel() 
{
	...
	RV_RealPlayType subtype;
	int subidx = m_normalBtnPannel.GetSubType();
	switch(subidx)
	{
	case 0:
		subtype = RV_RType_Realplay_0;
		break;
	case 1:
		subtype = RV_RType_Realplay_1;
		break;
	case 2:
		subtype = RV_RType_Realplay_2;
		break;
	case 3:
		subtype = RV_RType_Realplay_3;
		break;
	default:
		subtype = RV_RType_Realplay_0;
	}
	...
	if(nItem < 16)     //is channel item//是通道项
	{
	...
	else              //device item//设备项
	{
		OpenAllChannel((DeviceNode *)nItem, subtype);
	}
}

void CNetSDKDemoDlg::OpenSingleChannel(DeviceNode *pInfo, int nCh, int screenNo, RV_RealPlayType subtype)
{
	//查询是否被列入轮循列表
	//query if is been added to tour list
	...
	//关闭当前窗口的播放
	//close playing of current window
...
// 	nChannelID = CLIENT_RealPlay(nID, nCh, NULL);	
	nChannelID = CLIENT_RealPlayEx(nID, nCh, pWnd->m_hWnd/*m_playWnd[screenNo].m_hWnd*/);
	if(!nChannelID)
	{
		LastError();
	...
	//更新保存数据状态
	//upfate the status of saved data
...
//接收播放进度状态处理
//the deal of recving playing process status
...
//播放进度状态回调
//the callback of playing process status
...
{
	CSearchRecord dlg;
	if(dlg.DoModal() == IDOK)   //when opening replay//打开回放时
	{	
		if (!dlg.m_playfile) 
		{
			return;
...
//		plWnd->m_hWnd/*m_playWnd[m_curScreen].m_hWnd*/, PlayCallBack, (DWORD)this/*, PBDataCallBack, (DWORD)this*/);
	...
	{  //如果其它通道没有打开音频，则打开音频
		//if other channels not opened audio, open audio
...
//停止播放当前播放画面
//stop playing current playing view
...
int CNetSDKDemoDlg::PlayStop(int iScreen, BOOL bDis)
{
	...
	if(siNode.Type == SPLIT_TYPE_NETPLAY)
	{
		SplitNetPlayParam *pParam = (SplitNetPlayParam *)siNode.Param;
		//如果处于单帧播放状态，需先恢复
		//if frame by frame playing, restore first
		if(pParam->iStatus == STATUS_STEP)
		{
			ret = CLIENT_StepPlayBack(siNode.iHandle, TRUE);
			if (!ret)
...
//snapshot current watching channel, and it is need to verify that snapshot playing channel
void CNetSDKDemoDlg::CaptureScreen() 
{
	...
	//抓图保存文件名的输入
	//input file name that snapshotted save to file
...
//用来打开显示画面分割各显示区域的信息
//used to open display the area infomation of view split
...
void CNetSDKDemoDlg::PtzControl(int type, BOOL stop, int param)
{
	...
	switch(type) {
	//在主页面的控制
	//control in main page
	case RV_PTZ_UP_CONTROL  :    //up//上           
	case RV_PTZ_DOWN_CONTROL:		//down//下
	case RV_PTZ_LEFT_CONTROL:	//left//左
	case RV_PTZ_RIGHT_CONTROL:		//right//右
	case RV_PTZ_ZOOM_ADD_CONTROL:		//zoom//变倍
	case RV_PTZ_ZOOM_DEC_CONTROL:		
	case RV_PTZ_FOCUS_ADD_CONTROL:	//focus//调焦
	case RV_PTZ_FOCUS_DEC_CONTROL:	
	case RV_PTZ_APERTURE_ADD_CONTROL:	//aperture//光圈
	case RV_PTZ_APERTURE_DEC_CONTROL:
	case RV_EXTPTZ_LEFTTOP :
	case RV_EXTPTZ_RIGHTTOP :
	case RV_EXTPTZ_LEFTDOWN :
	case RV_EXTPTZ_RIGHTDOWN:
	...
	case RV_PTZ_POINT_LOOP_CONTROL :   //tour in prestes//点间轮循
	case RV_PTZ_LAMP_CONTROL:            //lamp//灯
	default:
...
//the deal while closing window, main need to release dynamic memory
...
{	
	...
	//关闭打开的回调数据保存文件
	//close the file that used to save callback data
	for(i = 0; i < 16 ; i++)
	{
		BOOL ret = GetSplitInfo_Main(i, &siNode);
		if (!ret)
	...
	CLIENT_Cleanup();    //close network sdk//关闭网络sdk
    LastError();
	//close network sdk//关闭网络sdk
...
void CNetSDKDemoDlg::ProcessDeleteDevice(DeviceNode *pDevice, BOOL bDelList, BOOL bDis)
{
	...
	//检查是否正在刷新该设备的报警信息
	//check if is refreshing the alarm infomation of the device
	...
	//关闭该设备的监视通道和播放通道
	//close the watching channel and playing channel
	for(i = 0; i < 16; i++)
	{
		BOOL ret = GetSplitInfo_Main(i, &siNode);
		if (!ret)
	...
	//			CCSLock lck(g_cs);
				...
				for (int j = 0; j < count; j++)
				{
					...
					//比较轮循列表中节点的设备名
					//compare the device name of node in tour list
					if ((DWORD)pDev == ((CycleChannelInfo *)temp)->dwDeviceID) 
					{	
						//是否刚好是正在播放的节点
						//if is the playing node
						...
						//不是当前播放节点，直接删除
						//not current play node, directly delete
						else
						{
							delete (CycleChannelInfo *)((SplitCycleParam *)siNode.Param)->pChannelList->GetAt(pos2);
							((SplitCycleParam *)siNode.Param)->pChannelList->RemoveAt(pos2);
						...
						//replay prevous one in current postion, for correctly play next on timer event
				...
				//if that's other device channel in list, swtich within half second
				if (siNode.Param) 
				{
					SetTimer(i, 500, NULL);
				}
	...
	//删除查询文件列表中该设备的文件项
	//delete the file item of the device in query file list
...
//断开当前选择的设备
//disconnect current selected device
...
//接收到实时数后需要做的处理
//the deal after recving real-time data 
...
//	g_splitinfo[nScreen].nKBPS +=  dwBufSize;
	...
	case 0:
		if(siNode.SavecbFileRaw)
		{
			...
			if (m_spFile)
			{
				...
				if (spret != 0)
				{
					int iiii = 0;//error, this case will not occrur now//error，这种情况现在不会出现
				}
				else
				{
					//这里应当维护一个变量标志当前是否已顺利获得I帧
					//here, need to mantance one variable that flag if is geted I frame
					DH_FRAME_INFO *frame = 0;
					while ((frame = DHSP_GetNextFrame(m_hSp)) != NULL)
					{
						//这里只提取视频帧
						//here only pick out video frams
						if (frame->nType != DH_FRAME_TYPE_VIDEO)
						{
							continue;
						}
						//0 == frame->nLength表示无效帧
						//0 == frame->nLength flag unvalid frame
						if (0 == frame->nLength) 
						{
							continue;
						}
						//0 == frame->nFrameLength数据丢失了，后面的p帧应该丢掉，不然会有马赛克
						//0 == frame->nFrameLength data lost，discard other P frames，or will be mosiac
	...
	case 1:
		...
		if(siNode.SavecbFileStd)
		{
			fwrite(pBuffer, 1, dwBufSize, siNode.SavecbFileStd);//保存为dav数据
		}
		*/
		///*
		{		
			...
			//每个文件含有125帧数据，播放5秒钟
			if(g_nFrameCount < 125)
			{				
				fwrite(pBuffer, 1, dwBufSize, file);//保存为dav数据
...
//显示设备列表中当前选项的详细信息
//display detail infomation of current selected item in device list
...
void CNetSDKDemoDlg::OpenMultiplay() 
{
	...
	int channelid = 0;
	RV_RealPlayType rType;
	if (IDOK == iRet)
	{
		channelid = dlg.m_channelId;
		rType = RV_RealPlayType(RV_RType_Multiplay_1+dlg.m_iPreviewType);
	}
	else
	{
		return;
...
BOOL CNetSDKDemoDlg::OpenTalk(BOOL bOpen)
{
	...
	nItem = m_devicelist.GetItemData(tmpnode);
	if(nItem < 16)     //channel item//是通道项
	{
		node_dev = m_devicelist.GetParentItem(tmpnode);
		pInfo = (DeviceNode *)m_devicelist.GetItemData(node_dev);
	}
	else              //device channel//设备项
	{
...
/*End: yehao(10857) */
//是否保存回调原始数据
//if save callback originality data
...
//		EnterCriticalSection(&g_csCycle);
		...
		//refresh view, can display normally if some view is closed
		CPlayWnd *plWnd;
		for (int curWin = 0; curWin < RV_MAX_CHANNUM; curWin++)
		{
		...
		for (int screenNo = 0; screenNo < RV_MAX_CHANNUM; screenNo++)
		{
			KillTimer(screenNo);
			...
			if (siNode.Type == SPLIT_TYPE_CYCLEMONITOR)
			{
				...
				//user select 1 channel, is the same as that open 1 watch channel
				//test，无论通道多少，一视同仁
				//test, how many channels, all the same
				...
				//用户选择了2个或2个以上通道，开始轮循
				//user selected 2 or more channels, start to tour
				else
				{
					LONG nChannelID = CLIENT_RealPlay(nID, nCh, plWnd->m_hWnd/*m_playWnd[screenNo].m_hWnd*/);
					if(!nChannelID)
					...
					//设置轮循位置为第一个
					//set the position of tour the first
					((SplitCycleParam *)siNode.Param)->iCurPosition = 0;
					...
					//设置轮循计时器，将秒转换为毫秒
					//set tour timer, translate second to millisecond
					UINT interval = ((SplitCycleParam *)siNode.Param)->iInterval;
					SetTimer((UINT)screenNo, interval * 1000, NULL);
					UpdateCurScreenInfo();
				}
			}
		}//end of for (int screenNo = 0; screenNo < RV_MAX_CHANNUM; screenNo++)
	//LeaveCriticalSection(&g_csCycle);
	}
}
...
//	CString strAlmType;
	...
	CString strTemp;
	BOOL isAlarm = FALSE; //affirm if new alarm occur//确定是否有新的报警
	switch(node.alarmType)
	{
	case RV_COMM_ALARM:
//	case RV_ALARM_ALARM_EX:
//	case RV_MOTION_ALARM_EX:
//	case RV_VIDEOLOST_ALARM_EX:
		{
...
//	case DH_DISKERROR_ALARM_EX:
		...
		break;
	case RV_SOUND_DETECT_ALARM:
//	case RV_SOUND_DETECT_ALARM_EX:
		{
...
void CNetSDKDemoDlg::Getversion() 
{
	...
	byte bVer[4] = {0};
	bVer[0] = (byte)(lVer & 0x000000ff);
	bVer[1] = (byte)((lVer & 0x0000ff00)>>8);
	bVer[2] = (byte)((lVer & 0x00ff0000)>>16);
	bVer[3] = (byte)((lVer & 0xff000000)>>24);
	CString strVer;
	strVer.Format("%d.%d.%d.%d", bVer[3], bVer[2], bVer[1], bVer[0]);
...
showBTwindow:
	...
	*/
	...
	//		&tmpinfo->starttime, &tmpinfo->endtime, GetDlgItem(IDC_SCREEN1 + m_curScreen )->m_hWnd, PlayCallBack, (DWORD)this);
		...
		{  //如果其它通道没有打开音频，则打开音频
			//if other channle not opened audio, open audio
			if (m_curSoundSplit < 0)
			{
		...
		//时间播放的进度条目前不做
		//the process bar of time play not valid now
...
void CNetSDKDemoDlg::OpenAllChannel(DeviceNode *pInfo, RV_RealPlayType subtype) 
{
	LONG nChannelID = 0;
	...
	int ScreenNum = CUR_MAXCHAN;//the sdk supported max view number//本SDK现在所能显示的最大画面数
	int FreeScreen = 0;
	DWORD freeFlag = 0;
	...
	if(chNum > CUR_MAXCHAN)//channel number device returned > channel number showed by SDK//设备返回通道数>SDK显示通道数
	{
	...
	else 
	{
		...
		if((chNum > FreeScreen))//view number spileh < channel number device returned//空余画面数<设备返回通道数
		{
...
//打开轮循列表中下一个通道
//open the next channel in tour list
...
//	EnterCriticalSection(&g_csCycle);
	...
	//取轮循列表的下一个通道值
	//get the next channel value in tour list
	CycleChannelInfo *tempnode;
	...
	//如果已经到了列表末尾，则返回列表头
	//if acheived the tail of list, back to the front of list
...
void CNetSDKDemoDlg::SeleteNormalPannel()			//display "common functions" function page//显示“常规功能”页面
{
	m_normalBtnPannel.ShowWindow(SW_SHOW);
	m_advanceBtnPannel.ShowWindow(SW_HIDE);
	m_saveDataPannel.ShowWindow(SW_HIDE);
...
void CNetSDKDemoDlg::SeleteAdvancePannel()			//display "advanced" function page//显示“高级”功能页面
{
...
void CNetSDKDemoDlg::SeletePTZPannel()				//display "PTZ Control" function page//显示“云台控制”功能页面
{
...
void CNetSDKDemoDlg::SeleteSaveDataPannel()		//display "save data" function page//显示“数据保存”功能页面
{
...
void CNetSDKDemoDlg::SeleteColorPannel()			//display "color adjust" function page//显示“颜色调整”功能页面
{
...
void CNetSDKDemoDlg::SeletePlayCtrlPannel()		//display "play Control" function page//显示“播放控制”功能页面
{
...
void CNetSDKDemoDlg::SeleteDevListPannel()			//display "device list" function page//显示“设备列表”功能页面
{
	m_devicelist.ShowWindow(SW_SHOW);
	m_colorPannel.ShowWindow(SW_HIDE);
	m_playctrlPannel.ShowWindow(SW_HIDE);
...
//窗口改变时的处理
//deal with window changing
void CNetSDKDemoDlg::OnSize(UINT nType, int cx, int cy) 
{
	CDialog::OnSize(nType, cx, cy);

	//窗口最小化与窗口大小无变化不处理
	//not deal with window minimize and window size no-change
	if ((cx ==0 && cy == 0) || 
...
void CNetSDKDemoDlg::UpdatePannelPosition()
{
	...
	m_devicelist.MoveWindow(m_pannelRect); //device list, not sub window//设备列表，非子窗口
	...
	m_screenRect.top = m_clientRect.top/* + 5*/;
	m_screenRect.bottom = m_clientRect.bottom/* - 20*/;
	m_screenRect.left = m_clientRect.left+210/* + 10*/;
	m_screenRect.right = m_clientRect.right;
	GetDlgItem(IDC_SCREEN_WINDOW)->MoveWindow(m_screenRect);
	
	m_pannelRect.top = m_clientRect.top + 100;
	m_pannelRect.bottom = m_clientRect.bottom - 263;
	m_pannelRect.right = m_clientRect.left + 205;
	m_pannelRect.left = m_pannelRect.right - 200;
	GetDlgItem(IDC_FRAME_PANNEL)->MoveWindow(m_pannelRect);	
	...
	m_colorPannel.MoveWindow(m_pannelRect);
	m_playctrlPannel.MoveWindow(m_pannelRect);
	m_devicelist.MoveWindow(m_pannelRect); //dvice list, not sub window//设备列表，非子窗口
	m_normalBtnPannel.MoveWindow(m_btnAreaRect);
	m_advanceBtnPannel.MoveWindow(m_btnAreaRect);
	m_ptzPannel.MoveWindow(m_btnAreaRect);
	m_runtimeMsgPannel.MoveWindow(m_runtimeMsgRect);
...
{
	if (nIndex < 0 || nIndex > RV_MAX_CHANNUM)
	{
		return FALSE;
	}
...
void CNetSDKDemoDlg::GetDeviceWorkstate()			//function of event hanlder for key of "work status of device"//"设备工作状态"按键响应函数
{
	DeviceNode *pDev = (DeviceNode *)GetCurDeviceInfo();
	if(pDev == NULL)
	{
...
void CNetSDKDemoDlg::DeviceControldisk()			//function of event hanlder for key of "disk control"//"硬盘控制"按键响应函数
{
	DeviceNode *pDev = (DeviceNode *)GetCurDeviceInfo();
	if(pDev == NULL)
	{
...
void CNetSDKDemoDlg::DeviceUserinfo()				//function of event hanlder for key of "users info"//"用户信息"按键响应函数
{
	DeviceNode *pDev = (DeviceNode *)GetCurDeviceInfo();
	if(pDev == NULL)
	{
...
// 	}
// 
// 	for (int i = 0; i<RV_MAX_ALARM_IN_NUM; i++)
// 	{
// 		for (int j=0; j<DH_N_WEEKS; j++)
// 		{
