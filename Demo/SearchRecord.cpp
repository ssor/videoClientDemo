...
#define ISQUERYTYPE2 0  //use the mode querying by single record//是否采用单记录查询模式
/////////////////////////////////////////////////////////////////////////////
	...
	: CDialog(CSearchRecord::IDD, pParent)
{
	...
	m_playfile = 0;
	m_playCount = 0;
	memset(m_playList, 0, RV_MAX_CHANNUM*sizeof(FileInfoNode));
...
void CSearchRecord::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MinMaxInt(pDX, m_channelsel, 1, RV_MAX_CHANNUM);
...
void CSearchRecord::OnSchStartsearch() 
{
	...
	if(m_channelsel <= 0 || m_channelsel > RV_MAX_CHANNUM)
	{
	...
	{
		//卡号查询，查询类型为4
		//query by card No., the query type is 4
		nSchType = 4;
...
/*查询接口有两种实现方式，这里使用第一种*/
/*there are two applied methods for query interface, and we use the first one*/
	if (!ISQUERYTYPE2) 
	{
		...
		//第二种接口实现
		//the second interface applied method
	else 
	{
		LONG lHandle;
...
int CSearchRecord::InitComboxCallback_Imp(const DeviceNode& node)
{
	...
	m_devicesel.SetItemData(nIndex, (DWORD)(&node)); //set save current device pointer//设置保存当前设备指针值
...
//mainly used to deal with the problems of process bars, and now just surpport downloading one file
...
void CSearchRecord::OnSchDownload() 
{
	...
	//get device position from resultlist, and then get device id by this value
	//更改为直接获取设备信息指针
	//change to get device infomation pointer directly
