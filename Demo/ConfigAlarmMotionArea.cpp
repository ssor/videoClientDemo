...
BOOL CConfigAlarmMotionArea::OnInitDialog() 
{
	...
	int blckWid = (m_rect.Width()-RV_MOTION_COL+1)/RV_MOTION_COL;
	m_blckWid = blckWid;
	int blckHght = (m_rect.Height()-RV_MOTION_ROW+1)/RV_MOTION_ROW;
	m_blckHght = blckHght;
	for (int i = 0; i < RV_MOTION_ROW; i++)
	{
		for (int j = 0; j < RV_MOTION_COL; j++)
		{
			m_block[i][j].Create(
			NULL, 
			NULL, 
...
void CConfigAlarmMotionArea::SetArea(BYTE *area)
{
	...
	memcpy(m_myArea, area, RV_MOTION_COL*RV_MOTION_ROW);
