...
void CUserManage::OnAddUser() 
{
	...
	if (dwData < RV_MAX_USER_NUM) //leaf node //叶节点 
	{
...
void CUserManage::OnDeleteUser() 
{
	...
	DWORD dwData = m_userList.GetItemData(node);
	if (dwData < RV_MAX_USER_NUM)
	{
		BOOL bRet = CLIENT_OperateUserInfoEx(m_dev->LoginID, 4, (void *)&m_user_info.userList[dwData], 0, 10000);
		if (!bRet)
		{
...
void CUserManage::OnDeleteGroup() 
{
	...
	USER_GROUP_INFO_EX *gpInfo = (USER_GROUP_INFO_EX *)dwData;
	BOOL bRet = CLIENT_OperateUserInfoEx(m_dev->LoginID, 1, (void *)gpInfo, 0, 10000);
	if (!bRet)
	{
		((CNetSDKDemoDlg *)AfxGetMainWnd())->LastError();
	}
