...
void CConfigComm::GetConfig()
{
	...
	DWORD retlen = 0;
	BOOL bRet = CLIENT_GetDevConfig(m_pDev->LoginID, RV_DEV_COMMCFG, 0,
...
void CConfigComm::OnSelchangeCombo232com() 
{
	...
	if (Idx232 < 0 || Idx232 >= RV_MAX_232_NUM) 
	{
		return;
	}
...
void CConfigComm::OnSelchangeCombo485com() 
{
	...
	if (Idx485 < 0 || Idx485 >= RV_MAX_DECODER_NUM) 
	{
		return;
	}
...
void CConfigComm::OnApply() 
{
	...
	BOOL bRet = CLIENT_SetDevConfig(m_pDev->LoginID, RV_DEV_COMMCFG, 0,
