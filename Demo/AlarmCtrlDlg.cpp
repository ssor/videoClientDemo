...
void CAlarmCtrlDlg::OnSaveIo() 
{
	...
	if(m_DeviceId)
	{
		...
		if(m_inNum)
		{
			nRet = CLIENT_IOControl(m_DeviceId, RV_ALARMINPUT, m_inState, sizeof(ALARM_CONTROL) *m_inNum );
			if (!nRet)
			{
		...
		if(m_outNum)
		{
			nRet = CLIENT_IOControl(m_DeviceId, RV_ALARMOUTPUT, m_outState, sizeof(ALARM_CONTROL) *m_outNum );
			if (!nRet)
			{
				((CNetSDKDemoDlg *)GetParent())->LastError();//Zhengdh 06.11.24
			}
...
BOOL CAlarmCtrlDlg::OnInitDialog() 
{
	...
	if(m_DeviceId)
	{
		nRet = CLIENT_QueryIOControlState(m_DeviceId,RV_ALARMINPUT,NULL,0,&m_inNum);
		if(nRet)
		{
			...
			nRet = CLIENT_QueryIOControlState(m_DeviceId,RV_ALARMINPUT,m_inState,sizeof(ALARM_CONTROL)*m_inNum,&m_inNum);
			if (!nRet)
			{
				((CNetSDKDemoDlg *)GetParent())->LastError();//Zhengdh 06.11.24
			}
		...
		//输出
		//output
		nRet = CLIENT_QueryIOControlState(m_DeviceId,RV_ALARMOUTPUT,NULL,0,&m_outNum);
		if(nRet)
		{
			...
			nRet = CLIENT_QueryIOControlState(m_DeviceId,RV_ALARMOUTPUT,m_outState,sizeof(ALARM_CONTROL)*m_outNum,&m_outNum);
			if (!nRet)
			{
				((CNetSDKDemoDlg *)GetParent())->LastError();//Zhengdh 06.11.24
			}
...
void CAlarmCtrlDlg::OnTrigger() 
{
	...
	for(int i = 0; i < m_inNum; i++)
	{
		...
		BOOL b = CLIENT_ControlDevice(m_DeviceId, RV_TRIGGER_ALARM_IN, &m_triggerIn[i], 1000);
		if (!b)
		{
