...
public:
	...
	//the return value of callback function : 1 end enum , 0 continue enum
	...
	//the returned value can't be store forever that reget it next time
	int  GetDev(LONG lLoginID, DeviceNode& node);

	//写入回调出来的报警数据
	//write the data got by callback
