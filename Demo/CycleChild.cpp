...
//	m_pSplitInfo = (void *)new SplitInfoNode;
	...
	//轮循间隔初始化为10秒
	//initialize the time of span to 10 seconds
...
void CCycleChild::SetItemData(int row, DWORD data)
{
	...
	{
		//初始化该窗口的轮循列表
		//initialize tour list of windows
		InitList(data);
		((CSpinButtonCtrl *)GetDlgItem(IDC_SPIN1))->EnableWindow(TRUE);
		((CEdit *)GetDlgItem(IDC_EDIT1))->EnableWindow(TRUE);
		return;
...
void CCycleChild::OnOK()
{
	...
	//输出轮循信息
	//output tour infomation
	if (m_cmParam) 
	{
		//读取用户设置的时间间隔
		//read the time span configed by user
		...
		{
			//输入的时间间隔不合法
			//the time span inputed not valid
			return ;
		}
		...
		//将本地信息回写到全局变量里
		//save local infomation to global variable
	...
	//原先在轮循，现在列表被清空的情况
	//the case of that, touring list before been cleared
