...
{
	USER_GROUP_INFO_EX ugInfo = {0};
	ugInfo.dwID = m_user_info->dwGroupNum + 1;
	GetDlgItem(IDC_NAME_EDIT)->GetWindowText(ugInfo.name, RV_USER_NAME_LENGTH);
	GetDlgItem(IDC_MEMO_EDIT)->GetWindowText(ugInfo.memo, RV_MEMO_LENGTH);
	...
	BOOL bRet = CLIENT_OperateUserInfoEx(m_dev->LoginID, 0/*type: add user group*/, &ugInfo, 0, 1000);
	if (!bRet)
	{
