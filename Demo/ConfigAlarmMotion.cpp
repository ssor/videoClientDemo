	...
	: CDialog(CConfigAlarmMotion::IDD, pParent)
{
	...
	//}}AFX_DATA_INIT
	m_pDev = 0;
	memset(m_motionCFG, 0, RV_MAX_VIDEO_IN_NUM*sizeof(RV_MOTION_DETECT_CFG));
	m_bReady = FALSE;
	m_chlIdx = -1;
	m_weekIdx = -1;
	m_ptzchlIdx = -1;
...
void CConfigAlarmMotion::SetMotionCFG(RV_MOTION_DETECT_CFG *cfg)
{
	if (!cfg)
	{
		return;
	...
	memcpy(m_motionCFG, cfg, RV_MAX_VIDEO_IN_NUM*sizeof(RV_MOTION_DETECT_CFG));

	DWORD dwActMask = m_motionCFG[0].struHandle.dwActionMask;
	//sentence if some functions enabled
	GetDlgItem(IDC_CHECK_UPLOAD)->EnableWindow((dwActMask & RV_ALARM_UPLOAD)?TRUE:FALSE);
	GetDlgItem(IDC_CHECK_LOCALTIP)->EnableWindow((dwActMask & RV_ALARM_TIP)?TRUE:FALSE);
	GetDlgItem(IDC_CHECK_LOCALTOUR)->EnableWindow((dwActMask & RV_ALARM_TOUR)?TRUE:FALSE);
	GetDlgItem(IDC_CHECK_SENDMAIL)->EnableWindow((dwActMask & RV_ALARM_MAIL)?TRUE:FALSE);
	
	//sentence if ptz enabled
	BOOL bPTZEn = (dwActMask & RV_ALARM_PTZ)?TRUE:FALSE;
	...
	BOOL bRecEn = (dwActMask & RV_ALARM_RECORD)?TRUE:FALSE;
	...
	BOOL bOutEn = (dwActMask & RV_ALARM_OUT)?TRUE:FALSE;
...
void CConfigAlarmMotion::OnSelchangeComboVideochl() 
{
	...
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_UPLOAD))->GetCheck()?RV_ALARM_UPLOAD:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_LOCALTIP))->GetCheck()?RV_ALARM_TIP:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_LOCALTOUR))->GetCheck()?RV_ALARM_TOUR:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_SENDMAIL))->GetCheck()?RV_ALARM_MAIL:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_PTZ))->GetCheck()?RV_ALARM_PTZ:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_RECORD))->GetCheck()?RV_ALARM_RECORD:0;
	m_motionCFG[m_chlIdx].struHandle.dwActionFlag |= ((CButton*)GetDlgItem(IDC_CHECK_OUTPUT))->GetCheck()?RV_ALARM_OUT:0;
...
skipsave:
	...
	((CButton*)GetDlgItem(IDC_CHECK_UPLOAD))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&RV_ALARM_UPLOAD)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_LOCALTIP))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&RV_ALARM_TIP)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_LOCALTOUR))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&RV_ALARM_TOUR)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_SENDMAIL))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&RV_ALARM_MAIL)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_PTZ))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&RV_ALARM_PTZ)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_RECORD))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&RV_ALARM_RECORD)?1:0);
	((CButton*)GetDlgItem(IDC_CHECK_OUTPUT))->SetCheck((m_motionCFG[chlIdx].struHandle.dwActionFlag&RV_ALARM_OUT)?1:0);
...
void CConfigAlarmMotion::OnBtnArea() 
{
	...
	{
		memcpy(m_motionCFG[m_chlIdx].byDetected, areaDlg.GetArea(), RV_MOTION_ROW*RV_MOTION_COL);
	}
}

void CConfigAlarmMotion::GetMotionCFG(RV_MOTION_DETECT_CFG *cfg)
{
	...
	memcpy(cfg, m_motionCFG, RV_MAX_ALARM_IN_NUM*sizeof(RV_MOTION_DETECT_CFG));
}
