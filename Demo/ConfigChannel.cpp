...
void CConfigChannel::GetConfig()
{
	...
	bRet = CLIENT_GetDevConfig(m_pDev->LoginID, RV_DEV_CHANNELCFG, -1/*all channel*/,
...
void CConfigChannel::OnSelchangeComboChanno() 
{
	...
	GetDlgItemText(IDC_EDIT_CHANNAME, m_chnCFG[m_chnIdx].szChannelName, RV_CHAN_NAME_LEN+1);
	BYTE rsOIdx, ecOIdx;
...
skipsave:
	...
	char tmpName[RV_CHAN_NAME_LEN+1] = {0};
	memcpy(tmpName, m_chnCFG[chnIdx].szChannelName, RV_CHAN_NAME_LEN);
...
//	m_chnCFG[1].stChannelOSD.rgbaFrontground = 0xFFFFFFFF;
	...
	BOOL bRet = CLIENT_SetDevConfig(m_pDev->LoginID, RV_DEV_CHANNELCFG, -1/*all channel*/,
